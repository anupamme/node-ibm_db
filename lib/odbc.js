...
// Expose convertRowsToColumns for use with prepare/bind/execute pattern
module.exports.convertRowsToColumns = convertRowsToColumns;
...
Database.prototype.query = function (query, params, cb)
{
  ...
  self.queue.push(function (next) {
    ...
    {
      var rowsErr = processRowsOption(query);
      if(rowsErr) {
        deferred ? deferred.reject(rowsErr) : cb(rowsErr);
        return next();
      }
      sql = query.sql;
    ...
    try {
      ...
      {
        // Resolve any Readable stream Data before proceeding
        resolveStreamParams(params, function(streamErr) {
          if (streamErr) {
            deferred ? deferred.reject(streamErr) : cb(streamErr);
            return next();
          }
...
Database.prototype.queryResult = function (query, params, cb)
{
  ...
  if(typeof query === "object")
  {
      var rowsErr = processRowsOption(query);
      if(rowsErr) {
        deferred ? deferred.reject(rowsErr) : ( cb && cb(rowsErr) );
        return deferred ? deferred.promise : false;
      }
      sql = query.sql;
      if(query.params) params = query.params;
...
Database.prototype.primaryKeys = function(catalog, schema, table, callback)
{
  var self = this, deferred;
  var error = {
      error : "[node-ibm_db] Missing Arguments",
      message : "The object you passed must contain three arguments: " +
                "catalog, schema and table. " +
                "This is required for the primaryKeys method to work."
    };
  if (!self.queue) self.queue = [];
  ...
  self.queue.push(function (next)
  {
    self.conn.createStatement(function (err, stmt)
    {
      ...
      stmt._primaryKeys(catalog, schema, table, function (err, rows)
      {
        stmt.closeSync();
        self.checkConnectionError(err);
        if (err) {
...
Database.prototype.primaryKeysSync = function(catalog, schema, table)
{
  var self = this;
  ...
  try {
    var rows = stmt._primaryKeysSync(catalog, schema, table);
    stmt.closeSync();
    return rows;
  } catch (e) {
...
Database.prototype.procedures = function(catalog, schema, procedure, callback)
{
  var self = this, deferred;
  var error = {
      error : "[node-ibm_db] Missing Arguments",
      message : "The object you passed must contain three arguments: " +
                "catalog, schema and procedure. " +
                "This is required for the procedures method to work."
    };
  if (!self.queue) self.queue = [];
  ...
  self.queue.push(function (next)
  {
    self.conn.createStatement(function (err, stmt)
    {
      ...
      stmt._procedures(catalog, schema, procedure, function (err, rows)
      {
        stmt.closeSync();
        self.checkConnectionError(err);
        if (err) {
...
Database.prototype.proceduresSync = function(catalog, schema, procedure)
{
  var self = this;
  ...
  try {
    var rows = stmt._proceduresSync(catalog, schema, procedure);
    stmt.closeSync();
    return rows;
  } catch (e) {
    try { stmt.closeSync(); } catch (_) {}
...
Database.prototype.procedureColumns = function(catalog, schema, procedure, column, callback)
{
  var self = this, deferred;
  var error = {
      error : "[node-ibm_db] Missing Arguments",
      message : "The object you passed must contain four arguments: " +
                "catalog, schema, procedure and column. " +
                "This is required for the procedureColumns method to work."
    };
  if (!self.queue) self.queue = [];
  ...
  self.queue.push(function (next)
  {
    self.conn.createStatement(function (err, stmt)
    {
      ...
      stmt._procedureColumns(catalog, schema, procedure, column, function (err, rows)
      {
        stmt.closeSync();
        self.checkConnectionError(err);
        if (err) {
...
Database.prototype.procedureColumnsSync = function(catalog, schema, procedure, column)
{
  var self = this;
  ...
  try {
    var rows = stmt._procedureColumnsSync(catalog, schema, procedure, column);
    stmt.closeSync();
    return rows;
  } catch (e) {
    try { stmt.closeSync(); } catch (_) {}
...
// cancel - Cancel an executing SQL statement
// This calls SQLCancel on the statement handle to cancel a long-running query.
// Must be called from a different thread/context than the executing query.
// Arguments: stmt - the statement object to cancel (required)
//            cb   - callback function (optional, returns Promise if omitted)
Database.prototype.cancel = function (stmt, cb)
{
  var self = this, deferred;

  if (!stmt || typeof stmt === 'function') {
    var err = new Error("cancel() requires a statement object as the first argument");
  ...
  self.queue.push(function (next)
  {
    exports.debug && console.log(getElapsedTime(), "odbc.js:cancel()");
    if (!self.connected) {
      deferred ? deferred.reject(connError) : cb(connError);
      return next();
    }
    ...
    try {
      self.conn.cancel(stmt, function (err, result)
      {
        if (err)
        {
          self.checkConnectionError(err);
      ...
      self.checkConnectionError(e);
      deferred ? deferred.reject(e) : cb(e);
      return next();
    }
  });
  if(deferred) return deferred.promise;
...
// cancelSync - Synchronous version of cancel
// Arguments: stmt - the statement object to cancel (required)
Database.prototype.cancelSync = function (stmt)
{
  var self = this;

  exports.debug && console.log(getElapsedTime(), "odbc.js:cancelSync()");

  if (!stmt) {
    throw new Error("cancelSync() requires a statement object");
  }
  ...
  try {
    return self.conn.cancelSync(stmt);
  } catch (e) {
    self.checkConnectionError(e);
    return e;
  }
...
if( !odbc.ODBCStatement.prototype._execute ) { //issue #514
  ...
  odbc.ODBCStatement.prototype._procedures = odbc.ODBCStatement.prototype.procedures;
  odbc.ODBCStatement.prototype._proceduresSync = odbc.ODBCStatement.prototype.proceduresSync;
  odbc.ODBCStatement.prototype._procedureColumns = odbc.ODBCStatement.prototype.procedureColumns;
  odbc.ODBCStatement.prototype._procedureColumnsSync = odbc.ODBCStatement.prototype.procedureColumnsSync;
  odbc.ODBCStatement.prototype._paramData = odbc.ODBCStatement.prototype.paramData;
  odbc.ODBCStatement.prototype._paramDataSync = odbc.ODBCStatement.prototype.paramDataSync;
  odbc.ODBCStatement.prototype._putData = odbc.ODBCStatement.prototype.putData;
  odbc.ODBCStatement.prototype._putDataSync = odbc.ODBCStatement.prototype.putDataSync;
  odbc.ODBCStatement.prototype._executeForStreaming = odbc.ODBCStatement.prototype.executeForStreaming;
  odbc.ODBCStatement.prototype._executeForStreamingSync = odbc.ODBCStatement.prototype.executeForStreamingSync;
  odbc.ODBCStatement.prototype._cancel = odbc.ODBCStatement.prototype.cancel;
  odbc.ODBCStatement.prototype._cancelSync = odbc.ODBCStatement.prototype.cancelSync;
  ...
  odbc.ODBCResult.prototype._fetchN = odbc.ODBCResult.prototype.fetchN;
 ...
 * Convert row-wise array data to column-wise ARRAY parameters.
 * rows: Array of arrays, where each inner array is one row of values.
 * columns (optional): Array of column descriptors with {DataType, CType, Length}.
 * Returns object with {params, ArraySize} on success, or {error} on failure.
 */
function convertRowsToColumns(rows, columns) {
    ...
    for (var r = 0; r < numRows; r++) {
        if (!Array.isArray(rows[r])) {
            return { error: "Each element in rows must be an array. " +
                            "Row " + r + " is not an array." };
        }
        if (numCols === null) {
            numCols = rows[r].length;
        } else if (rows[r].length !== numCols) {
            return { error: "All rows must have the same number of columns. " +
                            "Row " + r + " has " + rows[r].length +
                            " columns, expected " + numCols + "." };
        }
    }

    if (numCols === 0) {
        return { error: "Rows must have at least one column." };
    }

    if (columns && (!Array.isArray(columns) || columns.length !== numCols)) {
        return { error: "columns array length (" +
                        (Array.isArray(columns) ? columns.length : 0) +
                        ") must match the number of columns in rows (" +
                        numCols + ")." };
    }

    var params = [];
    for (var c = 0; c < numCols; c++) {
        var colData = new Array(numRows);
        for (var r = 0; r < numRows; r++) {
            colData[r] = rows[r][c];
        }

        var param = { ParamType: "ARRAY", DataType: 1, Data: colData };
        if (columns && columns[c]) {
            if (columns[c].DataType !== undefined) param.DataType = columns[c].DataType;
            if (columns[c].CType !== undefined) param.CType = columns[c].CType;
            if (columns[c].SQLType !== undefined) param.SQLType = columns[c].SQLType;
            if (columns[c].Length !== undefined) param.Length = columns[c].Length;
        }
        params.push(param);
    }

    return { params: params, ArraySize: numRows };
}

/*
 * processRowsOption
 * If the query object has a 'rows' property, convert row-wise data to
 * column-wise ARRAY params and set ArraySize. Mutates the query object.
 * Returns an error string on failure, or undefined on success.
 */
function processRowsOption(query) {
    if (typeof query !== 'object' || !query.rows) return;

    var result = convertRowsToColumns(query.rows, query.columns);
    if (result.error) return result.error;

    query.params = result.params;
    if (!query.ArraySize) query.ArraySize = result.ArraySize;
    delete query.rows;
...
function parseParams(params)
{
    ...
    for (var i = 0; i < params.length; i++)
    {
        if(Object.prototype.toString.call(params[i]) == "[object Object]")
        {
            ...
            else if(Array.isArray(prm.Data) && prm.Data.length > 0 &&
                    Buffer.isBuffer(prm.Data[0]))
            {
                // Array of Buffers: data-at-exec chunked LOB insert
                paramtype = 5; // DATA_AT_EXEC_PARAM
                if(!ctype) ctype = -2; // SQL_C_BINARY
                if(!sqltype) sqltype = -98; // SQL_BLOB
            }
...
/*
 * resolveStreamParams
 * Check if any param has a Readable stream as Data.
 * If so, collect all chunks from the stream and replace Data with
 * an array of Buffers. Calls cb(err) when done.
 * If no streams found, calls cb(null) immediately.
 */
function resolveStreamParams(params, cb) {
    if (!Array.isArray(params)) return cb(null);

    var pending = [];
    for (var i = 0; i < params.length; i++) {
        var p = params[i];
        if (p && typeof p === 'object' && !Array.isArray(p) &&
            !Buffer.isBuffer(p) && p.Data &&
            typeof p.Data === 'object' && p.Data !== null &&
            typeof p.Data.on === 'function' &&
            typeof p.Data.read === 'function')
        {
            pending.push({ index: i, stream: p.Data });
        }
    }
    ...
    for (var j = 0; j < pending.length; j++) {
        (function(item) {
            var chunks = [];
            item.stream.on('data', function(chunk) {
                if (!Buffer.isBuffer(chunk)) chunk = Buffer.from(chunk);
                chunks.push(chunk);
            });
            item.stream.on('end', function() {
                if (hasError) return;
              if (chunks.length === 0) chunks.push(Buffer.alloc(0));
                params[item.index].Data = chunks;
                remaining--;
                if (remaining === 0) cb(null);
            });
            item.stream.on('error', function(err) {
                if (!hasError) {
                    hasError = true;
                    cb(err);
                }
...
// Async function to get primary key columns for a table.
// stmt.primaryKeys(catalog, schema, table, callback) or
// stmt.primaryKeys(catalog, schema, table) => returns Promise
odbc.ODBCStatement.prototype.primaryKeys = function (catalog, schema, table, cb)
{
  var self = this, deferred;
  ...
  self.queue.push(function (next) {
    self._primaryKeys(catalog, schema, table, function (err, rows) {
      if (err) {
        deferred ? deferred.reject(err) : cb(err, []);
      } else {
...
// Sync function to get primary key columns for a table.
odbc.ODBCStatement.prototype.primaryKeysSync = function (catalog, schema, table)
{
  var self = this;
...
// Async function to get a list of procedure names.
// stmt.procedures(catalog, schema, procedure, callback) or
// stmt.procedures(catalog, schema, procedure) => returns Promise
odbc.ODBCStatement.prototype.procedures = function (catalog, schema, procedure, cb)
{
  var self = this, deferred;
  ...
  self.queue.push(function (next) {
    self._procedures(catalog, schema, procedure, function (err, rows) {
      if (err) {
        deferred ? deferred.reject(err) : cb(err, []);
      } else {
        deferred ? deferred.resolve(rows) : cb(null, rows);
...
// Sync function to get a list of procedure names.
odbc.ODBCStatement.prototype.proceduresSync = function (catalog, schema, procedure)
{
  var self = this;
  return self._proceduresSync(catalog, schema, procedure);
};

// Async function to get parameter information for a procedure.
// stmt.procedureColumns(catalog, schema, procedure, column, callback) or
// stmt.procedureColumns(catalog, schema, procedure, column) => returns Promise
odbc.ODBCStatement.prototype.procedureColumns = function (catalog, schema, procedure, column, cb)
{
  var self = this, deferred;
  ...
  self.queue.push(function (next) {
    self._procedureColumns(catalog, schema, procedure, column, function (err, rows) {
      if (err) {
        deferred ? deferred.reject(err) : cb(err, []);
      } else {
        deferred ? deferred.resolve(rows) : cb(null, rows);
...
// Sync function to get parameter information for a procedure.
odbc.ODBCStatement.prototype.procedureColumnsSync = function (catalog, schema, procedure, column)
{
  var self = this;
  return self._procedureColumnsSync(catalog, schema, procedure, column);
};

/*
 * paramData - Get next parameter for which a data value is needed.
 * This function calls SQLParamData and is used with putData for sending
 * large data in chunks or for data-at-execution parameters.
 * Returns object: { needsData: boolean, paramIndex: number|null }
 * If callback is not provided, returns a Promise.
 */
odbc.ODBCStatement.prototype.paramData = function (cb)
{
  var self = this, deferred;
  self.queue = self.queue || new SimpleQueue();
  ...
  self.queue.push(function (next) {
    self._paramData(function (err, result) {
      if (err) {
        if (self.db) self.db.checkConnectionError(err);
        deferred ? deferred.reject(err) : cb(err);
      } else {
        deferred ? deferred.resolve(result) : cb(null, result);
      }
...
/*
 * paramDataSync - Synchronously get next parameter for which a data value is needed.
 * Returns object: { needsData: boolean, paramIndex: number|null }
 */
odbc.ODBCStatement.prototype.paramDataSync = function ()
{
  var self = this;
  return self._paramDataSync();
};

/*
 * putData - Send data value for a parameter.
 * This function calls SQLPutData and is used with paramData for sending
 * large data in chunks or for data-at-execution parameters.
 * data: Buffer or String containing the data to send
 * length: Optional length of data (defaults to buffer/string length)
 * If callback is not provided, returns a Promise.
 */
odbc.ODBCStatement.prototype.putData = function (data, length, cb)
{
  var self = this, deferred;
  self.queue = self.queue || new SimpleQueue();
  ...
  self.queue.push(function (next) {
    var args = length !== undefined ? [data, length] : [data];
    ...
    self._putData.apply(self, args);
  });
  return deferred ? deferred.promise : null;
};

/*
 * putDataSync - Synchronously send data value for a parameter.
 * data: Buffer or String containing the data to send
 * length: Optional length of data (defaults to buffer/string length)
 * Returns true on success.
 */
odbc.ODBCStatement.prototype.putDataSync = function (data, length)
{
  var self = this;
  if (length !== undefined) {
    return self._putDataSync(data, length);
  }
  return self._putDataSync(data);
};

/*
 * executeForStreaming(callback)
 *
 * Execute the prepared statement but do NOT auto-complete the data-at-execution
 * loop. Returns { needsData: boolean } to indicate if paramData/putData calls
 * are required to send data for data-at-execution parameters.
 *
 * This is used for true streaming where data is sent incrementally via
 * paramData()/putData() rather than buffering all data upfront.
 */
odbc.ODBCStatement.prototype.executeForStreaming = function (cb)
{
  var self = this, deferred;
  self.queue = self.queue || new SimpleQueue();
  ...
  self.queue.push(function (next) {
    self._executeForStreaming(function (err, result) {
      if (err) {
        deferred ? deferred.reject(err) : cb(err);
      } else {
        deferred ? deferred.resolve(result) : cb(null, result);
 ...
 *
 * Synchronous version. Returns { needsData: boolean }.
 */
odbc.ODBCStatement.prototype.executeForStreamingSync = function ()
{
  var self = this;
  return self._executeForStreamingSync();
};

/*
 * executeWithStream(params, streamParamIndex, stream, cb)
 *
 * High-level helper for true streaming. Executes the statement and streams
 * data from a Readable stream to a data-at-execution parameter without
 * buffering the entire stream in memory.
 *
 * @param params - Array of parameters. The stream parameter should have
 *                 { ParamType: "INPUT", DataType: "BLOB"|"CLOB", Streaming: true }
 * @param streamParamIndex - Index of the parameter that will receive streamed data
 * @param stream - A Readable stream to read data from
 * @param cb - Callback function(err, result)
 *
 * Usage:
 *   stmt.prepareSync("INSERT INTO tbl (id, data) VALUES (?, ?)");
 *   stmt.bindSync([1, { ParamType: "INPUT", DataType: "BLOB", Streaming: true, Length: totalSize }]);
 *   stmt.executeWithStream(1, fs.createReadStream('largefile.bin'), function(err) {
 *     if (err) console.log(err);
 *     else console.log("Streaming insert complete");
 *   });
 */
odbc.ODBCStatement.prototype.executeWithStream = function (streamParamIndex, stream, cb)
{
  var self = this, deferred;

  if (typeof stream === 'function') {
    cb = stream;
    stream = streamParamIndex;
    streamParamIndex = 0; // default to first param
  }
  ...
  // Execute - this will return SQL_NEED_DATA if data-at-exec params exist
  self.executeForStreaming(function (err, execResult) {
    if (err) return done(err);

    if (!execResult.needsData) {
      // No data-at-exec params, nothing to stream
      return done(null, { streamed: false });
    }

    // Check which param needs data
    self.paramData(function (err, paramResult) {
      if (err) return done(err);

      if (!paramResult.needsData) {
        // Execution completed, no more data needed
        return done(null, { streamed: false });
      }

      // Stream data for this parameter
      var hasError = false;

      stream.on('data', function (chunk) {
        if (hasError) return;

        // Pause stream for backpressure
        stream.pause();
        ...
        // Send chunk via putData
        self.putData(chunk, function (err) {
          if (err) {
            hasError = true;
            stream.destroy();
            return done(err);
      ...
      stream.on('end', function () {
        ...
        // Call paramData to complete this param and move to next (or complete)
        self.paramData(function (err, result) {
          if (err) return done(err);

          if (result.needsData) {
            // More params need data - for simplicity, this version supports single stream
            return done(new Error("Multiple streaming params not supported in this call"));
          }

          done(null, { streamed: true, paramIndex: streamParamIndex });
        });
      });
...
// Cancel - Cancel an executing SQL statement on this statement handle.
// This can be called from another thread to cancel a long-running query.
// If no callback function, then return Promise.
odbc.ODBCStatement.prototype.cancel = function (cb)
{
  var self = this, deferred;
  self.queue = self.queue || new SimpleQueue();
  ...
  self.queue.push(function (next) {
    self._cancel(function (err, result) {
      if( err ) {
          self.db && self.db.checkConnectionError(err);
          deferred ? deferred.reject(err) : cb(err);
      } else {
          deferred ? deferred.resolve(result) : cb(null, result);
      }
...
// CancelSync - Synchronous version of cancel.
// Calls SQLCancel to cancel an executing statement.
odbc.ODBCStatement.prototype.cancelSync = function ()
{
  var self = this;
  try {
    return self._cancelSync();
  } catch (e) {
    self.db && self.db.checkConnectionError(e);
    throw e;
  }
};
...
// Async function to fetch up to N rows from the result set.
// result.fetchN(count, [option], callback);
// count: positive integer - maximum number of rows to fetch in this batch.
// option: optional object {fetchMode: 4|3} default is 4 (OBJECT).
// callback: optional - callback(err, rows). rows is an array.
// If callback is not provided, returns a Promise that resolves to the rows array.
// Does not close the cursor or free columns, so it can be called repeatedly.
odbc.ODBCResult.prototype.fetchN = function (count, option, cb) {
  var self = this, deferred;
  self.queue = self.queue || new SimpleQueue();
  ...
  {
    var error = { message: "fetchN(): count must be a positive integer." };
    deferred ? deferred.reject(error) : (cb && cb(error));
    return deferred ? deferred.promise : false;
  }
  ...
  self.queue.push(function (next) {
    var args = option ? [count, option] : [count];
...
// Async version of bindFileToColSync.
// Bind a LOB result column to an output file using SQLBindFileToCol.
// result.bindFileToCol(colNum, filePath, fileOption, callback);
// If callback is not provided, returns a Promise.
odbc.ODBCResult.prototype.bindFileToCol = function (colNum, filePath, fileOption, cb)
{
  var self = this, deferred;

  if (typeof fileOption === 'function') {
    cb = fileOption;
    fileOption = exports.SQL_FILE_OVERWRITE;
  }
  if (!cb) {
    deferred = createDeferred();
  }
  ...
  try {
    self.bindFileToColSync(colNum, filePath, fileOption);
    deferred ? deferred.resolve(true) : cb(null);
  } catch(err) {
    deferred ? deferred.reject(err) : cb(err);
  }
